...
export default function MyPage({ data }) {
  ...
  export async function getServerSideProps({ query }) {
    ...
    try {
    const response = await axios.get('https://egovernanceprojectwithnestjs-production.up.railway.app/doctor/finddoctor/'+inputValue);
    const data = await response.data;

    return { props: {data} };
    } catch (error) {
  
    return { props: { data: {status:"Enter valid user id"} }  };
  }
  }