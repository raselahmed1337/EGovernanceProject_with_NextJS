...
export default function MyPage({ data }) {
  ...
  return (
    <>
     <MyLayout title="Find Doctor By Phone Number"/>
...
<h1>Name: {data.name}</h1>
  ...
  export async function getServerSideProps({ query }) {
    ...
    try {
    const response = await axios.get('https://egovernanceprojectwithnestjs-production.up.railway.app/doctor/phone/'+inputValue);
    ...
    return {
      props: {
        data: {status:"enter valid user phone number"}
      }
    };
  }
  }